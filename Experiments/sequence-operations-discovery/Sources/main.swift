...
// Test: count.where, count.all
func testCount() {
    print("=== count.where/all ===")
    do {
        var container = NCContainer([1, 2, 3, 4, 5, 6])
        let evenCount = container.count.where { $0 % 2 == 0 }
        let totalCount = container.count.all
        print("  [1,2,3,4,5,6].count.where { even }: \(evenCount)")
        print("  [1,2,3,4,5,6].count.all: \(totalCount)")
