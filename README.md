...
- [swift-index](https://github.com/swift-atoms/swift-index) — `Index<Element>`, `Ordinal`, `Cardinal` (via index's re-export chain), the typed-indexing surface the iterator family builds on.
- [swift-property](https://github.com/swift-atoms/swift-property) — `Property<Tag, Base>` and `Property.Inout`, the phantom-tagged fluent-accessor machinery that powers `.contains { }`, `.satisfies.all { }`, `.forEach.borrowing { }`, and the rest of the terminal surface.
