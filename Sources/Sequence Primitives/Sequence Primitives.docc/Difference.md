...
The implementation ships in `Sequence Difference` as a separate sub-target. The diff types conform to `Sequence.Protocol`, so the result composes with the lazy/terminal surface — you can `.filter { $0.isChange }`, `.reduce.into(0) { ... }`, etc.
...
for hunk in hunks {
    print(hunk.header)  // e.g., "@@ -\(hunk.old.start),\(hunk.old.count) +\(hunk.new.start),\(hunk.new.count) @@"
